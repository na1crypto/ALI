...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$ism || !$tel || !$username || !$parol) {
        ...
        if (mysqli_stmt_num_rows($check) > 0) {
            ...
            // TiDB Cloud uchun id ni qo'lda beramiz
            $row   = mysqli_fetch_assoc(mysqli_query($conn, "SELECT COALESCE(MAX(id), 13) + 1 AS nid FROM users"));
            $newId = (int)$row['nid'];

            $stmt = mysqli_prepare($conn, "INSERT INTO users (id, name, username, password, phone, role) VALUES (?, ?, ?, ?, ?, ?)");
            mysqli_stmt_bind_param($stmt, 'isssss', $newId, $ism, $username, $ph, $tel, $role);
            ...
            if (mysqli_stmt_execute($stmt)) {
                ...
                $xato = "Xatolik: " . mysqli_stmt_error($stmt);
                mysqli_stmt_close($stmt);
            }
        }
    }
